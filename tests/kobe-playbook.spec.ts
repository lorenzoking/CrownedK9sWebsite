...
test.describe('Kobe go-home playbook (PPP buyer handoff)', () => {
	test('loads cover, shows dynamic age, and navigates slides', async ({ page }) => {
		await page.goto('/kobe-go-home-playbook/');

		await expect(page.locator('meta[name="robots"]')).toHaveAttribute('content', /noindex/);

		await expect(page.getByRole('heading', { level: 1, name: /Kobe/ })).toBeVisible();
		...
		await expect(page.getByRole('heading', { level: 2, name: /Potty training/i })).toBeVisible();

		await page.keyboard.press('ArrowLeft');
		await expect(page.getByRole('heading', { level: 1, name: /Kobe/ })).toBeVisible();

		// Cover image loads (not broken)
	...
	test('slide dots jump to expected sections', async ({ page }) => {
		await page.goto('/kobe-go-home-playbook/');
		await page.getByRole('button', { name: 'Go to slide 9' }).click();
		await expect(page.getByRole('heading', { level: 2, name: /not alone/i })).toBeVisible();
	});
